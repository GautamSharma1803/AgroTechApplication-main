...
const kvSet = async (key: string, value: any): Promise<void> => {
  ...
  const { error } = await client.from("kv_store_2598bc7a").upsert({
    key,
    value
  });
...
const kvGet = async (key: string): Promise<any> => {
  const client = kvClient();
  const { data, error } = await client.from("kv_store_2598bc7a").select("value").eq("key", key).maybeSingle();
  if (error) throw new Error(error.message);
  return data?.value;
};
...
const kvDel = async (key: string): Promise<void> => {
  const client = kvClient();
  const { error } = await client.from("kv_store_2598bc7a").delete().eq("key", key);
  if (error) throw new Error(error.message);
};

const kvGetByPrefix = async (prefix: string): Promise<any[]> => {
  const client = kvClient();
  const { data, error } = await client.from("kv_store_2598bc7a").select("key, value").like("key", prefix + "%");
  if (error) throw new Error(error.message);
...
app.get("/make-server-2598bc7a/crops", async (c) => {
  try {
    ...
    const crops = await kvGetByPrefix(`crops:${user.id}:`);
...
app.post("/make-server-2598bc7a/crops", async (c) => {
  try {
    ...
    await kvSet(`crops:${user.id}:${cropId}`, crop);
    return c.json({ crop });
  } catch (error) {
...
app.put("/make-server-2598bc7a/crops/:id", async (c) => {
  try {
    ...
    const existing = await kvGet(`crops:${user.id}:${cropId}`);
    if (!existing) {
...
app.delete("/make-server-2598bc7a/crops/:id", async (c) => {
  try {
    ...
    const cropId = c.req.param("id");
    await kvDel(`crops:${user.id}:${cropId}`);
...
app.post("/make-server-2598bc7a/diagnose", async (c) => {
  try {
    ...
    if (!PLANT_ID_API_KEY) {
      ...
      await kvSet(`diagnosis:${user.id}:${diagnosisId}`, diagnosis);
...
app.get("/make-server-2598bc7a/diagnoses", async (c) => {
  try {
    ...
    const diagnoses = await kvGetByPrefix(`diagnosis:${user.id}:`);
...
app.post("/make-server-2598bc7a/soil-tests", async (c) => {
  try {
    ...
    await kvSet(`soiltest:${user.id}:${testId}`, soilTest);
...
app.get("/make-server-2598bc7a/soil-tests", async (c) => {
  try {
    ...
    const tests = await kvGetByPrefix(`soiltest:${user.id}:`);
...
app.get("/make-server-2598bc7a/market/products", async (c) => {
  try {
    ...
    const products = await kvGetByPrefix("market:product:");
...
app.post("/make-server-2598bc7a/market/products", async (c) => {
  try {
    ...
    await kvSet(`market:product:${productId}`, product);
...
app.get("/make-server-2598bc7a/market/my-listings", async (c) => {
  try {
    ...
    const allProducts = await kvGetByPrefix("market:product:");
